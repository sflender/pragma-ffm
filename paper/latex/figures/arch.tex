...
\begin{tikzpicture}[
  ...
  x/.style={-{Latex[length=2mm]}, thick},
...
\draw[x] (fields) -- (evt);
\draw[x] (evt) -- (hist) node[midway,above,font=\scriptsize] {[EVT]};
\draw[x] (hist) -- (head) node[midway,above,font=\scriptsize] {record emb.};
\draw[x] (head) -- (out);
...
\node[att, right=of nbr, minimum height=10mm] (xseq)
  {Cross-Sequence Encoder {\scriptsize(attn 3)}\\[1pt]
   {\scriptsize pattern (attn) $+$ log-count}};
\draw[x] (nbr) -- (xseq);
\draw[x, dashed] (nbr.north) to[out=90,in=180] (evt.west|-nbr.north) ;
\draw[x] (evt.south) to[out=-90,in=180] ([yshift=2mm]xseq.west);
\draw[x] (xseq.north) to[out=90,in=-90] (hist.south)
...
Encoder, with a pattern (attention) path and an always-on log-count (magnitude) path.}
\label{fig:arch}
\end{figure}
