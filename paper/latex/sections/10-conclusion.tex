...
while a frozen masked backbone cannot (the proxy-alignment gap). On real click fraud it beats the
per-account FFM by \num{+0.055} PR-AUC. The relational signal comes in two forms --- a neighbour
\emph{pattern} and a velocity \emph{count} --- and a \emph{count-aware} cross-sequence encoder
carrying both paths wins in each regime with a single module. A map across four real datasets says
